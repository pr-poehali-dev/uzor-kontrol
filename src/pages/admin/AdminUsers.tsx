...
import { MOCK_ADMIN_USERS, AdminUser } from '@/lib/admin-data';
...
const planBadge = (p: AdminUser['plan']) => {
  const map = { free: 'neutral', pro: 'info', business: 'success' } as const;
  return <Badge variant={map[p]}>{p}</Badge>;
};

const statusBadge = (s: AdminUser['status']) => {
  const map = { active: 'success', blocked: 'error', expired: 'warning' } as const;
...
export default function AdminUsers() {
  ...
  const filtered = useMemo(() => {
    ...
    return users.filter(u =>
      ...
      (u.email.toLowerCase().includes(q) || u.name.toLowerCase().includes(q) || u.country.toLowerCase().includes(q))
    );
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <PageHeader
        title="Users"
        description={`${MOCK_ADMIN_USERS.length} total · ${MOCK_ADMIN_USERS.filter(u => u.status === 'active').length} active`}
      />
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-white/10">
                ...
                <th className="text-left px-4 py-3.5 text-xs text-muted-foreground font-medium uppercase tracking-wider hidden lg:table-cell">Connections</th>
            ...
            <tbody className="divide-y divide-white/5">
              {filtered.map(user => (
                <tr key={user.id} className="hover:bg-white/3 transition-colors group">
                  ...
                  <td className="px-4 py-4 text-muted-foreground hidden md:table-cell">{formatDate(user.registeredAt)}</td>
                  <td className="px-4 py-4 text-muted-foreground hidden lg:table-cell">{timeAgoLabel(user.lastSeen)}</td>
